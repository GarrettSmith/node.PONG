...
/*!
  ...
  * License MIT
  */
(function(context){function require(identifier){var module=modules["$"+identifier]||window[identifier];if(!module)throw new Error("Ender Error: Requested module '"+identifier+"' has not been defined.");return module}function provide(name,what){return modules["$"+name]=what}function aug(o,o2){for(var k in o2)k!="noConflict"&&k!="_VERSION"&&(o[k]=o2[k]);return o}function Ender(s,r){var elements,i;this.selector=s;if(typeof s=="undefined"){elements=[];this.selector=""}else typeof s=="string"||s.nodeName||s.length&&"item"in s||s==window?elements=ender._select(s,r):elements=isFinite(s.length)?s:[s];this.length=elements.length;for(i=this.length;i--;)this[i]=elements[i]}function ender(s,r){return new Ender(s,r)}context.global=context;var modules={},old=context.$,oldEnder=context.ender,oldRequire=context.require,oldProvide=context.provide;context.provide=provide;context.require=require;Ender.prototype.forEach=function(fn,opt_scope){var i,l;for(i=0,l=this.length;i<l;++i)i in this&&fn.call(opt_scope||this[i],this[i],i,this);return this};Ender.prototype.$=ender;ender._VERSION="0.4.3-dev";ender.fn=Ender.prototype;ender.ender=function(o,chain){aug(chain?Ender.prototype:ender,o)};ender._select=function(s,r){return typeof s=="string"?(r||document).querySelectorAll(s):s.nodeName?[s]:s};ender.noConflict=function(callback){context.$=old;if(callback){context.provide=oldProvide;context.require=oldRequire;context.ender=oldEnder;typeof callback=="function"&&callback(require,provide,this)}return this};typeof module!="undefined"&&module.exports&&(module.exports=ender);context.ender=context.$=ender}(this));(function(){function _addEvent(object,type,callback){if(object.addEventListener)return object.addEventListener(type,callback,!1);object.attachEvent("on"+type,callback)}function _characterFromEvent(e){return e.type=="keypress"?String.fromCharCode(e.which):_MAP[e.which]?_MAP[e.which]:_KEYCODE_MAP[e.which]?_KEYCODE_MAP[e.which]:String.fromCharCode(e.which).toLowerCase()}function _stop(e){var element=e.target||e.srcElement,tag_name=element.tagName;return(" "+element.className+" ").indexOf(" mousetrap ")>-1?!1:tag_name=="INPUT"||tag_name=="SELECT"||tag_name=="TEXTAREA"||element.contentEditable&&element.contentEditable=="true"}function _modifiersMatch(modifiers1,modifiers2){return modifiers1.sort().join(",")===modifiers2.sort().join(",")}function _resetSequences(do_not_reset){do_not_reset=do_not_reset||{};var active_sequences=!1,key;for(key in _sequence_levels){if(do_not_reset[key]){active_sequences=!0;continue}_sequence_levels[key]=0}active_sequences||(_inside_sequence=!1)}function _getMatches(character,modifiers,action,remove,combination){var i,callback,matches=[];if(!_callbacks[character])return[];action=="keyup"&&_isModifier(character)&&(modifiers=[character]);for(i=0;i<_callbacks[character].length;++i){callback=_callbacks[character][i];if(callback.seq&&_sequence_levels[callback.seq]!=callback.level)continue;if(action!=callback.action)continue;if(action=="keypress"||_modifiersMatch(modifiers,callback.modifiers)){remove&&callback.combo==combination&&_callbacks[character].splice(i,1);matches.push(callback)}}return matches}function _eventModifiers(e){var modifiers=[];e.shiftKey&&modifiers.push("shift");e.altKey&&modifiers.push("alt");e.ctrlKey&&modifiers.push("ctrl");e.metaKey&&modifiers.push("meta");return modifiers}function _fireCallback(callback,e){if(callback(e)===!1){e.preventDefault&&e.preventDefault();e.stopPropagation&&e.stopPropagation();e.returnValue=!1;e.cancelBubble=!0}}function _handleCharacter(character,e){if(_stop(e))return;var callbacks=_getMatches(character,_eventModifiers(e),e.type),i,do_not_reset={},processed_sequence_callback=!1;for(i=0;i<callbacks.length;++i){if(callbacks[i].seq){processed_sequence_callback=!0;do_not_reset[callbacks[i].seq]=1;_fireCallback(callbacks[i].callback,e);continue}!processed_sequence_callback&&!_inside_sequence&&_fireCallback(callbacks[i].callback,e)}e.type==_inside_sequence&&!_isModifier(character)&&_resetSequences(do_not_reset)}function _handleKey(e){e.which=typeof e.which=="number"?e.which:e.keyCode;var character=_characterFromEvent(e);if(!character)return;if(e.type=="keyup"&&_ignore_next_keyup==character){_ignore_next_keyup=!1;return}_handleCharacter(character,e)}function _isModifier(key){return key=="shift"||key=="ctrl"||key=="alt"||key=="meta"}function _resetSequenceTimer(){clearTimeout(_reset_timer);_reset_timer=setTimeout(_resetSequences,1e3)}function _getReverseMap(){if(!_REVERSE_MAP){_REVERSE_MAP={};for(var key in _MAP){if(key>95&&key<112)continue;_MAP.hasOwnProperty(key)&&(_REVERSE_MAP[_MAP[key]]=key)}}return _REVERSE_MAP}function _pickBestAction(key,modifiers,action){action||(action=_getReverseMap()[key]?"keydown":"keypress");action=="keypress"&&modifiers.length&&(action="keydown");return action}function _bindSequence(combo,keys,callback,action){_sequence_levels[combo]=0;action||(action=_pickBestAction(keys[0],[]));var _increaseSequence=function(e){_inside_sequence=action;++_sequence_levels[combo];_resetSequenceTimer()},_callbackAndReset=function(e){_fireCallback(callback,e);action!=="keyup"&&(_ignore_next_keyup=_characterFromEvent(e));setTimeout(_resetSequences,10)},i;for(i=0;i<keys.length;++i)_bindSingle(keys[i],i<keys.length-1?_increaseSequence:_callbackAndReset,action,combo,i)}function _bindSingle(combination,callback,action,sequence_name,level){combination=combination.replace(/\s+/g," ");var sequence=combination.split(" "),i,key,keys,modifiers=[];if(sequence.length>1)return _bindSequence(combination,sequence,callback,action);keys=combination==="+"?["+"]:combination.split("+");for(i=0;i<keys.length;++i){key=keys[i];_SPECIAL_ALIASES[key]&&(key=_SPECIAL_ALIASES[key]);if(action&&action!="keypress"&&_SHIFT_MAP[key]){key=_SHIFT_MAP[key];modifiers.push("shift")}_isModifier(key)&&modifiers.push(key)}action=_pickBestAction(key,modifiers,action);_callbacks[key]||(_callbacks[key]=[]);_getMatches(key,modifiers,action,!sequence_name,combination);_callbacks[key][sequence_name?"unshift":"push"]({callback:callback,modifiers:modifiers,action:action,seq:sequence_name,level:level,combo:combination})}function _bindMultiple(combinations,callback,action){for(var i=0;i<combinations.length;++i)_bindSingle(combinations[i],callback,action)}var module={exports:{}},exports=module.exports,_MAP={8:"backspace",9:"tab",13:"enter",16:"shift",17:"ctrl",18:"alt",20:"capslock",27:"esc",32:"space",33:"pageup",34:"pagedown",35:"end",36:"home",37:"left",38:"up",39:"right",40:"down",45:"ins",46:"del",91:"meta",93:"meta",224:"meta"},_KEYCODE_MAP={106:"*",107:"+",109:"-",110:".",111:"/",186:";",187:"=",188:",",189:"-",190:".",191:"/",192:"`",219:"[",220:"\\",221:"]",222:"'"},_SHIFT_MAP={"~":"`","!":"1","@":"2","#":"3",$:"4","%":"5","^":"6","&":"7","*":"8","(":"9",")":"0",_:"-","+":"=",":":";",'"':"'","<":",",">":".","?":"/","|":"\\"},_SPECIAL_ALIASES={option:"alt",command:"meta","return":"enter",escape:"esc"},_REVERSE_MAP,_callbacks={},_direct_map={},_sequence_levels={},_reset_timer,_ignore_next_keyup=!1,_inside_sequence=!1;for(var i=1;i<20;++i)_MAP[111+i]="f"+i;for(i=0;i<=9;++i)_MAP[i+96]=i;_addEvent(document,"keypress",_handleKey);_addEvent(document,"keydown",_handleKey);_addEvent(document,"keyup",_handleKey);var mousetrap={bind:function(keys,callback,action){_bindMultiple(keys instanceof Array?keys:[keys],callback,action);_direct_map[keys+":"+action]=callback;return this},unbind:function(keys,action){if(_direct_map[keys+":"+action]){delete _direct_map[keys+":"+action];this.bind(keys,function(){},action)}return this},trigger:function(keys,action){_direct_map[keys+":"+action]();return this},reset:function(){_callbacks={};_direct_map={};return this}};module.exports=mousetrap;provide("mousetrap",module.exports);$.ender(module.exports)})();(function(){var module={exports:{}},exports=module.exports;(function(undefined){function _encode(data,top){if(typeof data=="number"){var m=data|0;if(m!==data){var add=0,r=(data-m)*100;if(r<0){add=(r+1|0)-r;r=add>=1&&add<=1.5?r|0:r-1|0}else{add=r|0;r=r-add>=.5?r+1|0:add}add=0;if(data<0){m=0-m;r=0-r;add=1}m<65536?m===0?enc+=tok[13+add]+tok[r+128]:enc+=tok[13+add]+tok[r]+tok[m]:enc+=tok[15+add]+tok[m>>16&65535]+tok[m&65535]+tok[r]}else{var add=0;if(data<=0){data=0-data;add=1}else data--;data<116?enc+=tok[17+data+add*116]:data<65536?enc+=tok[1+add]+tok[data]:enc+=tok[3+add]+tok[data>>16&65535]+tok[data&65535]}}else if(typeof data=="string"){var l=data.length;enc+=tok[7];while(l>=65535){l-=65535;enc+=tok[65535]}enc+=tok[l]+data}else if(data===!0)enc+=tok[5];else if(data===!1)enc+=tok[6];else if(data===null)enc+=tok[0];else if(data instanceof Array){enc+=tok[8];for(var i=0,l=data.length;i<l;i++)_encode(data[i],!1);top||(enc+=tok[9])}else if(data instanceof Object){enc+=tok[10];for(var e in data){enc+=tok[17+e.length]+e;_encode(data[e],!1)}top||(enc+=tok[11])}}function encode(data){enc="";_encode(data,!0);return enc}function decode(data){var p=0,l=data.length,stack=[],dec=undefined,f=null,t=0,i=-1,dict=!1,set=!1,key="",e=null,r=0;while(p<l){t=data.charCodeAt(p++);f=stack[i];if(dict&&set&&t>16){key=data.substring(p,p+t-17);p+=t-17;set=!1}else if(t===8||t===10){e=t===8?new Array:new Object;set=dict=t===10;dec!==undefined?f instanceof Array?f.push(e):f[key]=e:dec=e;stack.push(e);i++}else if(t===11||t===9){stack.pop();set=dict=!(stack[--i]instanceof Array)}else if(t>16){t-=17;t=t>115?0-t+116:t+1;f instanceof Array?f.push(t):f[key]=t;set=!0}else if(t>0&&t<5){if(((t-1)/2|0)===0){e=data.charCodeAt(p);p++}else{e=(data.charCodeAt(p)<<16)+data.charCodeAt(p+1);p+=2}e=t%2?e+1:0-e;f instanceof Array?f.push(e):f[key]=e;set=!0}else if(t>12&&t<17){if(((t-13)/2|0)===0){r=data.charCodeAt(p);if(r>127){e=0;r-=128;p++}else{e=data.charCodeAt(p+1);p+=2}}else{e=(data.charCodeAt(p)<<16)+data.charCodeAt(p+1);r=data.charCodeAt(p+2);p+=3}e=t%2?e+r*.01:0-(e+r*.01);f instanceof Array?f.push(e):f[key]=e;set=!0}else if(t>4&&t<7){f instanceof Array?f.push(t===5):f[key]=t===5;set=!0}else if(t===0){f instanceof Array?f.push(null):f[key]=null;set=!0}else if(t===7){e=0;while(data.charCodeAt(p)===65535){e+=65535;p++}e+=data.charCodeAt(p++);f instanceof Array?f.push(data.substr(p,e)):f[key]=data.substr(p,e);p+=e;set=!0}}return dec}var chr=String.fromCharCode,tok=new Array(65536);for(var i=0;i<65536;i++)tok[i]=chr(i);var enc="";if(typeof window=="undefined"){exports.encode=encode;exports.decode=decode}else window.BISON={encode:encode,decode:decode}})();provide("bison",module.exports);$.ender(module.exports)})();(function(){var module={exports:{}},exports=module.exports;
/*!
